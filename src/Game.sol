...
contract Game is ERC721 {
    ...
    function putUpForSale(uint256 _monId) external {
        require(
            ownerOf(_monId) == msg.sender,
            "Can only put your own mons up for sale"
        );
        forSale[_monId] = true;
    }
    ...
    function indexInDeck(address _owner, uint256 _monId)
        public
        view
        returns (uint256 idx)
    {
    ...
    function swapForNewMon(uint256 _monId) external {
        address swapper = msg.sender;
        require(
            ownerOf(_monId) == swapper,
            "Can only swap your own Mon for a new Mon"
        );
